...
| `--project` + `--compile` | Forward + Zod runtime validators |
...
| `--compile` | | Emit Zod validators (`fromJSONSchema`). Requires `--output` and `zod` in consumer project |
...
# Generate with Zod runtime validators
