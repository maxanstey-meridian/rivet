...
| `Rivet*EnumConverter<T>` | enum | The string-enum converter family (requires .NET 9+, used inside `[JsonConverter(typeof(...))]`): `RivetLowerCaseEnumConverter<T>` / `RivetCamelCaseEnumConverter<T>` / `RivetSnakeCaseEnumConverter<T>` / `RivetKebabCaseEnumConverter<T>`. The converter class name declares both the string wire and the casing convention for enum wire values; the emitted contract values equal the runtime serializer's wire values by construction. A per-member `[JsonStringEnumMemberName]` overrides the casing. Two members casing to the same wire value (RIV1106) emit the enum as numeric. The bare built-in `[JsonConverter(typeof(JsonStringEnumConverter<T>))]` keeps exact C# member names. |
