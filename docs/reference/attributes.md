...
| `[RivetEnumNamingPolicy(RivetNamingPolicy.Policy)]` | enum | Names the casing convention for enum wire values on a string-wire enum (one that also declares `[JsonConverter(typeof(JsonStringEnumConverter<...>))]`): `LowerCase`, `CamelCase`, `SnakeCase`, `KebabCase`. Emission-only; a per-member `[JsonStringEnumMemberName]` overrides it. Without a string converter (RIV1105) or when the policy cases two members to the same wire value (RIV1106), the enum is emitted as numeric. |
