...
[RivetType]
public sealed record Either<TLeft, TRight>(TLeft? Left, TRight? Right, bool IsLeft);
...
[RivetType]
public sealed record TreeNode(string Label, List<TreeNode> Children);

[RivetType]
public sealed record Category(Guid Id, string Name, Category? Parent, List<Category> SubCategories);

// ========== Mutually referential ==========

[RivetType]
public sealed record Folder(string Name, List<Folder> SubFolders, List<Document> Documents);

public sealed record Document(string Title, DateTime CreatedAt, Folder Parent);
...
[RivetType]
public sealed record Comparable<T>(T Value, Comparable<T>? Next)
    where T : notnull;
...
[RivetType]
public sealed record TaggedResult<T>(T Data, (string Source, DateTime FetchedAt) Meta);
