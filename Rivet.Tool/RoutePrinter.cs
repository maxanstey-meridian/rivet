...
internal static class RoutePrinter
{
    public static void Print(IReadOnlyList<TsEndpointDefinition> endpoints)
    {
        ...
        var sorted = endpoints.OrderBy(e => e.RouteTemplate).ThenBy(e => e.HttpMethod).ToList();
        ...
        Console.WriteLine(
            $"  {methodLabel.PadRight(methodWidth)}  {routeLabel.PadRight(routeWidth)}  {handlerLabel}"
        );
        Console.WriteLine(
            $"  {"".PadRight(methodWidth, '─')}  {"".PadRight(routeWidth, '─')}  {"".PadRight(handlerLabel.Length, '─')}"
        );
        ...
        foreach (var ep in sorted)
        {
            ...
            var coloredRoute = Regex.Replace(route, @"\{[^}]+\}", m => $"\x1b[33m{m.Value}\x1b[0m");
            Console.WriteLine(
                $"  {methodColor}{method}\x1b[0m  {coloredRoute}  \x1b[90m{ep.ControllerName}.{ep.Name}\x1b[0m"
            );
        }
