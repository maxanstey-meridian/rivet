...
public static partial class ClientEmitter
{
    ...
    public static string EmitRivetBase()
    {
        ...
        sb.AppendLine("};");
        sb.AppendLine();
        sb.AppendLine("export const rivetFetch = async <T>(");
        ...
        bool validated = false)
    {
        ...
        // Import rivetFetch from shared module
    ...
    private static void EmitEndpoint(StringBuilder sb, TsEndpointDefinition endpoint, bool validated)
    {
        ...
        if (validated && endpoint.ReturnType is not null)
        {
            ...
            sb.AppendLine($"export const {funcName} = async ({paramsStr}): Promise<{returnTypeStr}> => {{");
        ...
        {
            sb.AppendLine($"export const {funcName} = ({paramsStr}): Promise<{returnTypeStr}> =>");
            sb.AppendLine($"  rivetFetch<{returnTypeStr}>(\"{endpoint.HttpMethod}\", `{route}`{optionsStr});");
        }
        else
        {
            sb.AppendLine($"export const {funcName} = ({paramsStr}): Promise<void> =>");
