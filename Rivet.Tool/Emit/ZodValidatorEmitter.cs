...
public static class ZodValidatorEmitter
{
        ...
        IReadOnlyDictionary<string, string> typeFileMap)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("// Bridge as-const readonly schemas to fromJSONSchema's mutable parameter type.");
        sb.AppendLine("type JSONSchemaInput = Parameters<typeof fromJSONSchema>[0];");
        sb.AppendLine("const toSchema = (s: unknown): JSONSchemaInput => s as JSONSchemaInput;");
    ...
    private static string BuildZodExpression(TsType type)
    {
        return type switch
        {
            TsType.TypeRef r => $"fromJSONSchema(toSchema({r.Name}Schema))",
            TsType.Generic g => $"fromJSONSchema(toSchema({JsonSchemaEmitter.MonomorphisedName(g)}Schema))",
            TsType.Brand b => $"fromJSONSchema(toSchema({b.Name}Schema))",
    ...
    private static string BuildPrimitiveSchema(TsType.Primitive p)
    {
        ...
        return $"fromJSONSchema(toSchema({json}))";
    }
