...
internal static class EmitPipeline
{
    ...
    internal static async Task<int> RunAsync(EmitInput input, RivetOptions options)
    {
        ...
        if (options.Verify)
        {
            ...
            {
                Console.Error.WriteLine(
                    $"error: --verify: {specPath} does not exist — run the same command without --verify to generate it"
                );
                return 1;
            }
            ...
            {
                Console.Error.WriteLine(
                    $"error: --verify: {specPath} is stale — the source no longer matches it. Regenerate (run without --verify) and commit the result."
                );
                return 1;
            }
        ...
        if (specPath is not null)
        {
            ...
            if (!options.Quiet)
            {
                ...
                Console.WriteLine(
                    $"Generated OpenAPI spec: {definitionsByName.Count} schemas, {endpoints.Count} endpoints."
                );
            }
        }
