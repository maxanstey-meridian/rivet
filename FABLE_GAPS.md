...
| Binary **upload** (raw body) | ✅ **closed in 0.37.0** — `.AcceptsBinary(contentType)`: spec-only binary requestBody, TInput lowers to route/query, imports back from `format: binary` bodies. (`Stream`/`byte[]` inputs still mislower — use `.AcceptsBinary()`.) | ⚠(misleading) — `input: Blob` → late `TYPE_NOT_FOUND "Blob"` far from the cause | C# surface added 0.37.0; the model field `binaryRequestContentType` survives contract JSON, so the TS lowerer can adopt it. | TS: **b** |
