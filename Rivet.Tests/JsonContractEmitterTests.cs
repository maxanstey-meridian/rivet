...
public sealed class JsonContractEmitterTests
{
    ...
    public void OpenApiEmitter_FromJson_EmitsSpec()
    {
        ...
        var idParam = Assert.Single(
            getOp.GetProperty("parameters").EnumerateArray(),
        ...
        var statusParam = Assert.Single(
            listOp.GetProperty("parameters").EnumerateArray(),
            p => p.GetProperty("name").GetString() == "status"
        );
        Assert.Equal("query", statusParam.GetProperty("in").GetString());
        var list200Schema = listOp
        ...
        Assert.Equal(
            "integer",
            productDto.GetProperty("properties").GetProperty("id").GetProperty("type").GetString()
        );
        Assert.Equal(
            "string",
            productDto
