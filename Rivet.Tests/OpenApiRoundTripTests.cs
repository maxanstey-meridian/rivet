...
public sealed class OpenApiRoundTripTests
{
    ...
    /// </summary>
    [Fact]
    public void MaximalContract_DoubleRoundTrip_IsLossless()
    {
        var source = ConformanceSources.MaximalContract;
        ...
        Assert.True(
            wlk2.Brands["Email"].Inner is TsType.Primitive { Name: "string" },
            $"Email brand inner should be string, got {wlk2.Brands["Email"].Inner}"
        );
        Assert.True(
            wlk2.Brands["Quantity"].Inner is TsType.Primitive { Name: "number" },
            $"Quantity brand inner should be number, got {wlk2.Brands["Quantity"].Inner}"
        );
        ...
        var searchResult = Assert.IsType<TsType.Generic>(searchEp.ReturnType);
        Assert.Equal("PagedResult", searchResult.Name);
        Assert.Equal(
            "KitchenSinkDto",
            Assert.IsType<TsType.TypeRef>(Assert.Single(searchResult.TypeArguments)).Name
        );

        var usersListEp = eps2.First(e => e.HttpMethod == "GET" && e.RouteTemplate == "/api/users");
        var usersResult = Assert.IsType<TsType.Generic>(usersListEp.ReturnType);
        Assert.Equal("PagedResult", usersResult.Name);
        Assert.Equal(
            "UserDto",
            Assert.IsType<TsType.TypeRef>(Assert.Single(usersResult.TypeArguments)).Name
        );
        ...
        Assert.Single(patchItem.Params, p => p.Source == ParamSource.Route && p.Name == "id");
        ...
        Assert.True(
            uploadEp.ReturnType is TsType.TypeRef { Name: "UploadResult" },
            $"Upload return type should be UploadResult, got {uploadEp.ReturnType}"
        );
        Assert.Contains(uploadEp.Responses, r => r.StatusCode == 201);
        ...
        {
            Assert.StartsWith("#/components/", refValue);
            var segments = refValue.Split('/');
            Assert.Equal(4, segments.Length);
            Assert.True(
                components.TryGetProperty(segments[2], out var section)
                    && section.TryGetProperty(segments[3], out _),
